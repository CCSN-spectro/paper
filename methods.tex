...
\begin{figure}[t]
 ...
 \caption{Ratio $M_{\rm PNS}/R_{\rm PNS}^2$ from our \pcd{18} 1D simulations of the model set. The solid line is the maximum likelihood estimate of heteroscedastic cubic model with 95\% confidence bands (dashed lines) considering the 18 simulation data points.  } 
 \label{fig:LMVAR}
\end{figure}
...
\cite{lmvar:2019} \pcd{[PCD: reference is empty]} that implements a maximum likelihood approach was used to fit the model.
...
the grey bands represent 95\% confidence bands. The size of the red triangles is proportional to the magnitude of the $\mbox{}^2 g_2$-mode \pcd{frequency?} estimates.  
\pcd{[PCD: By magnitude you mean the value of the frequency? ]}
