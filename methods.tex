...
An example is given in Figure \ref{fig:ratio} where the red triangles \sout{points} are the point estimates and
the grey bands represent 95\% confidence bands. {\color{red} The size of the red triangles are proportional to the magnitude of the $\mbox{}^2 g_2$-mode estimates.  Note that high $\mbox{}^2 g_2$-mode values have associated more uncertainty (bigger intervals) because our model allows heterogeneous variance.} Ratio values
...
\begin{figure}
 ...
 \caption{Ratio $M_{\rm PNS}/R_{\rm PNS}^2$ as function of time {\color{red}estimated} \sout{extracted} from the $\mbox{}^2 g_2$-mode of the {\texttt s20S} signal (red {\color{red}triangles} \sout{points} and the 95\% confidence belt in grey) compared to the ratio value derived from the PNS mass and radius given by the simulation code (black points).  Estimated ratios are represented proportionally to the magnitude of the $\mbox{}^2 g_2$-mode estimates. } \label{fig:ratio}
\end{figure}
