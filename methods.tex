...
\begin{figure}[t]
 ...
 \caption{Ratio $M_{\rm PNS}/R_{\rm PNS}^2$ from our 25 1D simulations of the model set. The solid line is the maximum likelihood estimate of heteroscedastic cubic model with 95\% confidence bands (dashed lines) considering the 18 simulation data points. \tf{18 or 25?} \tf{We could remove the legend as only one code is used.} } 
 \label{fig:LMVAR}
\end{figure}
...
signal is embedded in noise. \tf{We also note that despite we have only explicitely shown results for the GW signal of the {\texttt s20S} model the same conclusions hold for any of the other waveforms of our test set.}
