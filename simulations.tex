...
\pcd{Unless other methods used GW astronomy, the} algorithm proposed in \pcd{this work} does not require accurate
waveforms \pcd{in order to infer the properties of the PNS.} \pcd{Instead, it relies on the evolution of the frequency of
...
\pcd{We have used two different numerical codes in our numerical simulations.} 
%CoCoNuT
...
\pcd{For the {\it model set}, we use the $25$ spherically symmetric (1D) simulations of \citep{Torres:2019a}
...
$M_{\mathrm{ZAMS}} = 11.2 - 75 \, \Msol$. The set contains simulations using the
 ...
 \begin{table}
  ...
  \end{tabular}
  \caption{%%
    List of axisymmetric simulations \pcd{used for the {\it test set}}.  
    \pcd{The last three columns show, the post-bounce time at the end of the
...
\pcd{For the {\it test set}, we use $8$ axisymmetric (2D) simulations using the AENUS-ALCAR code
...
(GShen) for the \pcd{progenitor} with $M_{\mathrm{ZAMS}} = 15 \, \Msol$.  To this set of
...
\pcd{For all the simulations,} we mapped the pre-collapse state of the stars to a spherical
