...
Next, we use the universal relations obtained by~\citep{Torres:2019b}, based on a set of 1D simulations, to infer the time evolution of the ratio $M_{\rm PNS}/R_{\rm PNS}^2$ (the PNS surface gravity), where $M_{\rm PNS}$ is the mass of the PNS and $R_{\rm PNS}$ is the radius where the density is lower than \unit[$10^{11}$]{$\rm g/cm^3$}.
%  where  $M_{\rm PNS}$ and $R_{\rm PNS}$ are the mass and the radius of the PNS.
This same mode has been interpreted in some works as a f or a p-mode
