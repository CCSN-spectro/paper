...
%For the results discussed in this work we only consider neutrino-driven CCSN of non-rotating stellar cores.%\tf{This last sentence must be mentioned somewhere (or maybe not) but perhaps here is not the best place.}\mab{MAB:I'm in favor of deleting it as it is rather obvious we are considering neutrino driven CCSN}
...
Many previous data analysis efforts focused on the reconstruction of the gravitational wave strain amplitude
without assuming a particular signal model \citep{Summerscales:2008,Klimenko:2015ypf,CornLitt}. As numerical
simulations became more available other methods using catalogs of waveforms have been proposed to
identify the supernova explosion mechanism.
Among them, principal component analysis helps at reducing the complexity of CCSN waveform catalogs to fewer
